...
# Network: an interactive run asks whether to bind loopback-only (SSH tunnel) or
# public (0.0.0.0, reachable at http://<ip>:PORT). Use --local / --public to pick
# non-interactively. Only applies when generating a fresh .env.
set -euo pipefail
...
if [ -f .env ]; then
  ...
  # via --local/--public; non-interactive runs default to safe loopback.
  if [ "$BIND_MODE" = "ask" ]; then
    if [ -t 0 ]; then
      ...
      echo "  2) Public on this server's IP (0.0.0.0) — http://<ip>:$PORT_VAL"
      printf "Choose [1/2] (default 1): "
      read -r ans </dev/tty 2>/dev/null || ans=""
      case "$ans" in 2|p|P|public) BIND_MODE=public ;; *) BIND_MODE=local ;; esac
    else
      BIND_MODE=local
    fi
  fi
  ...
  if [ "$BIND_MODE" = "public" ]; then
    ...
    HOST_VAL=127.0.0.1
    ORIGINS_LINE=""   # omit the line so config.js's loopback default applies
  fi
  ...
  if [ "$BIND_MODE" = "public" ]; then
    warn "Public bind — dashboard at http://$PUB_IP:$PORT_VAL (open the firewall for $PORT_VAL)."
    warn "This is PLAIN HTTP: the AUTH_TOKEN travels unencrypted. Use only on a trusted"
    warn "network/VPN, or front it with a TLS reverse proxy (see DEPLOYMENT.md) and point"
    warn "ALLOWED_ORIGINS at your https:// URL."
  fi
fi
...
if [ "${HOST:-127.0.0.1}" = "0.0.0.0" ] || [ "${HOST:-}" = "::" ]; then
